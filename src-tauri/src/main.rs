...
use eFlow_lib::{ext_mod, tauri_generate_context};
...
fn main() -> Result<Infallible, Box<dyn Error>> {
    ...
    // 👉 Equivalent to `python -m eFlow`,
    // i.e, run the `src-tauri/python/eFlow/__main__.py`
    let py_script = PythonScript::Module("eFlow".into());
